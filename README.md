...
测试前可以先为观察机 B 配置同一来源的允许规则，确认基线可达；测试时将该规则改为拒绝，
或新增一条更高优先级的拒绝规则；测试完成后再恢复允许。不要把来源写成目标机 IP，来源
必须是发起连接的观察机地址。
...
如果目标机仍关联包含以下放行规则的其他安全组，或者平台对同安全组流量存在默认放行，单组
拒绝规则可能不会生效：
...
此时再考虑拆分观察机和目标机的安全组，作为排除默认互通或多安全组放行的手段。无论使用
一组还是两组，最终都要在云控制台的“关联实例”页面确认规则确实作用于目标机 A。
...
- 已确认目标机 A 的所有关联安全组中没有覆盖本次拒绝规则的全放行规则；如平台存在同安全组
  默认互通，也要以实际探测结果为准。
- 如果目前只有一台 Linux 服务器，停止这项测试；先准备另一台服务器或本地电脑作为观察机。
...
第二步，在**网页云控制台**关闭目标机 A 的 TCP `22` 入方向规则。不要通过目标机 A 的 SSH
终端修改规则。等待规则生效后，回到观察机 B 执行：
...
第三步，在**网页云控制台**原样恢复目标机 A 的 TCP `22` 允许规则。等待规则生效后，仍在
观察机 B 执行：
