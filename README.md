...
测试 SSH：  共享安全组（两台服务器共享一个安全组）拒绝 B -> A 的 TCP 22 -> 观察机B执行 blocked 命令 -> 恢复允许 -> 执行 restored 命令
```
...
### 场景二：拒绝和恢复观察机 B 访问 Linux 服务器 SSH 22

这个场景测试的是 Linux 服务器的入方向 TCP `22`，不是 Redis 的 `10.0.1.12:6379`。
如果 `10.0.1.12` 是托管 Redis 地址，不能把它的 `22` 当作 SSH 端口测试。

> **高风险操作：观察机和目标机必须是两台不同的机器。严禁在当前 SSH 登录的目标机上运行
> 测试脚本后再关闭该目标机的 `22`。关闭后当前终端可能立即卡死，无法继续输入恢复命令。**

测试拓扑必须是：

```text
云控制台                  修改和恢复两台共用的安全组中 B -> A 的 TCP 22 规则
观察机 B（Ubuntu，10.0.1.13）  运行 redis_instance_test.py，整个过程保持可操作
目标机 A（Debian，10.0.0.10）   被测试的 Linux 服务器，接收来自观察机 B 的 TCP 22 探测
```
...
两台机器不要求使用两个安全组，若共享一个安全组也可以完成测试。
> **共享安全组注意：** 当前环境中观察机 B 和目标机 A 共用一个安全组，因此在控制台可以把这次
> 操作理解为“拒绝观察机 B 访问 TCP:22”，所以后面就是需要关掉观察机B的22端口。由于该安全组
> 同时关联 Debian A，实际效果是阻断`10.0.1.13 -> 10.0.0.10:22`；这不是关闭观察机 B 自己的 SSH 服务。
> 若两台机器不共用安全组，则必须在目标机 A 的安全组中配置同样的拒绝规则。
...
备注：SSH 阻断测试
```
> **安全组规则顺序：** `TCP:22 拒绝` 必须排在会放行所有协议端口的规则之前，尤其要排在
> `0.0.0.0/0 ALL 允许` 或同类全放通规则之前，否则全放通规则可能先匹配，`22` 仍会显示
> `observed=reachable`。多数云服务控制台提供“排序”或规则优先级功能，调整后要保存并等待
> 生效，再用脚本验证；不要只看页面顺序，最终以实际探测结果为准。

可以先在准备作为观察机 B 的服务器上检查公网 IP：
...
以下三条测试命令全部在**观察机 B**执行。
第一步，在关闭规则前确认目标机 A 的 `22` 可达：
...
第二步，在**网页云控制台**的共享安全组中拒绝观察机 B（`10.0.1.13`）访问 TCP `22`。
由于该安全组也关联目标机 A，这会阻断 `10.0.1.13 -> 10.0.0.10:22`。不要通过目标机 A
的 SSH 终端修改规则。等待规则生效后，回到观察机 B 执行：
...
预期是 `expected=blocked observed=blocked` 和 `[PASS] security_group`。目标机 A 上已有的 SSH
会话可能卡死或断开，这是预期现象，不能回到该会话中执行后续命令。

第三步，在**网页云控制台**的共享安全组中，原样恢复观察机 B 访问目标机 A 的 TCP `22`
允许规则。等待规则生效后，仍在观察机 B 执行：
...
恢复验证通过后，再从本地电脑新建到目标机 A 的 SSH 连接，不要依赖旧会话。如果误操作导致
SSH 卡死，应立即从云控制台删除 TCP `22` 拒绝规则并恢复允许规则，等待生效后新建 SSH 连接。

安全组测试只能说明 TCP 当前可达或阻断，不能仅凭一次 TCP 失败区分安全组、网络 ACL、路由、
防火墙或 SSH 服务未监听；规则变更和原因定位仍需结合云平台配置及服务器状态。
